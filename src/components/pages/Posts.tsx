        ...
        <div className="flex flex-col lg:max-w-[950px] max-w-full lg:gap-y-8 gap-y-12 mb-12">
          {posts.map((post, index) =>
            post.isPublished !== true ? null : (
              <Slide key={post._id} delay={(index % 4) * 0.1}>
                <article>
                  <Link
                    href={`/blog/${post.slug}`}
                  ...
                  >
                    <div className="relative lg:w-[450px] lg:h-52 w-full h-48 sm:h-56 overflow-clip">
                      <Image
                        src={post.coverImage ? urlFor(post.coverImage).width(800).url() : fallbackImage}
