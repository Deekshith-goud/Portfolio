  ...
  useEffect(() => {
    const fetchContributions = async () => {
      try {
        ...
        const cacheKey = `v3_github_contributions_${username}`;
        ...
        if (cached) {
          ...
          const res = await fetchWithTimeout(`/api/github/calendar`);
          if (!res.ok) throw new Error("Failed to fetch");
          activities = await res.json();
          sessionStorage.setItem(cacheKey, JSON.stringify(activities));
        }

        const allContributions = Array.isArray(activities) 
          ? activities 
          : (activities as any)?.contributions?.flat() || [];
        ...
        const pastContributions = allContributions.filter((c: any) => {
          const dateStr = typeof c.date === 'string' ? c.date.split('T')[0] : c.date;
          return dateStr <= todayStr;
        });

        const last91Days: DayActivity[] = pastContributions.slice(-91).map((c: any) => {
          const rawDate = c.date;
          const dateObj = new Date(rawDate);
          const formattedDate = dateObj.toLocaleDateString("en-US", { month: "short", day: "numeric" });
          const count = typeof c.count === 'number' ? c.count : (c.contributionCount || 0);
          return {
            date: formattedDate,
            count: count,
            fullDate: rawDate
        ...
        const sorted = [...last91Days].filter((d: DayActivity) => d.count > 0).sort((a, b) => b.count - a.count);
        const top5Set = new Set(sorted.slice(0, 5).map((d: DayActivity) => d.fullDate));
        
        const finalData = last91Days.map((d: DayActivity) => ({
          ...d,
